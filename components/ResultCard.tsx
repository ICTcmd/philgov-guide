...
export default function ResultCard({ result, agency, action, isMock }: ResultCardProps) {
        ...
        <p key={i} className="mb-2 min-h-[1.2em]">
          {tokens.map((t, j) => {
            if (t.match(/^(https?:\/\/|www\.)/)) {
              ...
              return (
                <a key={j} href={href} target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:underline break-all">
                  {t}
                </a>
              );
  ...
  const downloadPdf = () => {
    ...
    // Clean text for PDF (remove markdown bold markers and headers)
    ...
    doc.text("Generated by BAGO APP AI - Verify with official sources.", 20, pageHeight - 10);
    
    doc.save(`BagoApp-${agency.replace(/\s+/g, '-')}.pdf`);
  };
  ...
  return (
    <div className="w-full animate-in fade-in zoom-in duration-500">
      ...
      <div className="relative h-full flex flex-col">
        {result && (
          <div className="flex flex-wrap justify-end gap-2 mb-2">
            <button
              ...
              className="text-xs font-medium bg-white hover:bg-emerald-50 text-emerald-700 border border-emerald-200 py-1.5 px-3 rounded-lg shadow-sm dark:bg-gray-700 dark:text-emerald-300 dark:border-gray-600 dark:hover:bg-gray-600 transition-all flex items-center gap-1"
            >
        ...
        <div className="p-6 w-full text-sm text-gray-800 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200 min-h-[500px] whitespace-pre-wrap leading-relaxed">
